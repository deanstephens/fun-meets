...
### Milestone 31 — Shared whiteboard ✅
- A **Whiteboard** action puts you in draw mode: sketch **freehand** on the
  shared board with a small toolbar (six pen colours, **Undo**, **Clear**,
  **Done**). Strokes broadcast to everyone in real time and a **late joiner sees
  the existing drawing** (the host syncs it). Ink rests on the board behind the
  avatars; in draw mode the canvas lifts to the front so you can draw anywhere.
  Strokes are stored as vector points, so they stay crisp and reflow on resize.

### Milestone 32 — Kickable ball ✅
- A **Ball** action drops a ⚽ on the board (or resets it to the centre); another
  removes it. **Walk into it to kick it** — it rolls off, bounces off the walls,
  and decelerates to a stop. The **host runs the physics** (position + velocity,
  wall + avatar collisions) and broadcasts state; everyone else dead-reckons
  between updates so motion stays smooth and consistent. Late joiners see the
  ball where it is.

### Milestone 33 — Tag minigame ✅
- A **Tag: start** action kicks off a game of tag — you become **"it"** (a red
  pulsing ring + "IT" badge on your avatar). **Touch another avatar to pass it
  on**; a short grace period stops instant tag-backs. The **host runs the game**
  (detecting tags from the synced positions) and broadcasts who's "it", so
  everyone — including late joiners — sees the same state. The tag reach is set
  above the collision distance, so it still works with avatar collision on.
  **Tag: stop** ends it.

### Milestone 34 — Presence status ✅
- Set a **status** shown under your name on your tile — quick **Available / Away
  / Busy** presets or **custom text** — from the Settings panel. It broadcasts to
  everyone (and reaches **late joiners**, same pattern as usernames), **persists
  across reloads**, and received statuses are **sanitised and length-capped**.

### Milestone 35 — Presence heartbeat (seamless brief blips) ✅
- Membership is now **explicit** rather than inferred from the transport: every
  peer sends a lightweight **presence ping**, and a clean leave sends a **"bye"**.
  A **very brief blip** that closes the data channel no longer drops-and-rejoins
  — the peer just shows **reconnecting** briefly and recovers (data + media are
  re-dialled). A **clean leave still drops promptly** (via the bye, given a short
...
### Milestone 37 — Whisper / direct message ✅
- Type **`/w <name> <message>`** (or `/whisper`) in the chat box to send a
  **private 1:1 message** to one participant over the data channel
  (`session.sendTo`) — only they receive it. It's shown in the chat panel on both
  ends **clearly marked private** (🔒, distinct styling), separate from room
  chat. Name matching is case-insensitive (longest match, so multi-word names
  work); an unmatched name just shows you a local hint.

### Milestone 38 — Microphone mute ✅
- A **Mute** button in the top bar toggles your mic (disables the audio track, so
  the connection stays up). A **🔇 badge** appears on your tile for everyone —
...
### Milestone 40 — iOS viewport / keyboard fixes ✅
- The stage is now sized and pinned to the **visible viewport** (via the
  `visualViewport` API with `dvh`/`vh` fallbacks) instead of `100vh`, so nothing
  is **cut off at the bottom** on iPhone landscape, and opening the **soft
  keyboard** shrinks the stage so the bottom controls/chat ride up above it
  rather than throwing the page off. The 💬 touch button also opens the (collapsed)
  sidebar so chat is reachable.

### Milestone 41 — Help / cheat-sheet overlay ✅
- Press **?** (or the touch **?** button) for a help overlay: the **controls**
  (move, carry, chat, whisper, emoji, mute) and the **full list of `/` actions**,
  generated **from the live action registry** so it never drifts. Escape, the ×,
  or tapping outside closes it.

### Milestone 42 — Raise hand + speaker queue ✅
- A **Raise hand** toggle (top bar, or the `/` action) puts a waving **🖐 badge**
  on your tile and adds you to an **ordered queue** panel everyone sees — sorted
  by when each hand went up (using the raiser's own clock so the order is
  identical for all). Broadcast like presence/mute, handed to **late joiners**;
  lowering your hand or leaving removes you from the queue.

### Milestone 43 — Connection-quality indicator ✅
- Each remote tile shows a small **quality pip** — green (good) / amber (ok) /
  red (poor) — based on your own connection's **round-trip time and recent
  packet loss** (polled from `RTCPeerConnection.getStats()` every few seconds).
  It degrades on a bad link and recovers when it improves; hover for the
  numbers. Purely local measurement — nothing extra over the wire.

### Milestone 44 — Confetti + sound effects ✅
- A **confetti burst** celebrates shared moments — a **board-game win** or a
  **poll close** — for everyone at once (driven by the existing synced state, so
  no extra traffic). A **Sound FX** toggle in Settings (persisted) adds little
  WebAudio cues — a win fanfare, a dice roll, the timer chime — all best-effort
  and non-blocking.

### Milestone 45 — Spotlight the active speaker ✅
- When **Spotlight speaker** is on (Settings, persisted), whoever is talking gets
  a stronger emphasis on their tile — a **brighter, larger glow + a gentle
  scale-up + raised z-order** — that fades in/out as the speaker changes, driven
  by the existing voice-activity detection. Turning it off restores the plain
  talking ring. Local/visual only.

### Milestone 46 — Persistent rooms ✅
- A room now **remembers itself**. The per-room board snapshot (#34) was
  extended to also cover **whiteboard strokes** and **board games** (on top of
  cards, huddle zones, and background), and every change to those persists. When
  a **host (re)joins an empty room**, the snapshot is **restored**; late joiners
  then receive it via the normal host sync. An already-active room is never
  clobbered (only the host restores, and only into a not-yet-settled board).
